...
// OJO: el backend expone este recurso en /api/receptores (RecipientController).
// La versión anterior de este formulario llamaba a /api/pacientes, que no existe.
const API_URL = `${BASE_URL}/api/receptores`
const MEDICOS_URL = `${BASE_URL}/api/medicos`

function PacienteRegistrationForm() {
  // Médico que registra (core.recipients.registered_by -> auth.users.id).
  // El backend no acepta email/password aquí: el paciente no se crea su
  // propia cuenta, lo registra un médico ya existente.
